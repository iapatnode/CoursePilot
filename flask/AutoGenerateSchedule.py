...
class Course:
    def __init__(self, courseCode, semesterAvail, courseName, creditHours, prerequisites, prerequisiteDepth, timeAvail, dayAvail):
...
def AutoGeneratedSchedule():
    ...
    numcourses = 0
    for courseRequired in CoursesRequired:
        #print("%s | %s" % (courseRequired.courseName, courseRequired.semesterAvail))
        numcourses += 1
    #print(numcourses)
    ...
    totalTime = 0            
   # print(len(recommendedSchedule))
    for course in recommendedSchedule:
        print(course.courseName)
        totalTime += course.creditHours
    print(totalTime)
...
def getRequiredCourses():
    ...
    try:
        ...
        for val in info:
            newCourse = Course(val[0], val[1], val[2], val[3], [], 0, str(val[4]) + str(val[5]), val[6])

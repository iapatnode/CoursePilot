...
def GetAutoGeneratedSchedule(user_email, semester_selection):
    ...
    ClassTimes = getRequiredClasses(CoursesRequired)
            ...
            for course in requiredCourses:
                if course.courseCode == prereq[2]:
                    for reqCourse in requiredCourses:
                        if (reqCourse == prereq[1]):
                            course.prerequisites.append(reqCourse)
...
def getRequiredClasses(CoursesRequired):
    try:
        cursor = conn.cursor()
...
#GetAutoGeneratedSchedule("dybasjt17@gcc.edu", "fall")
